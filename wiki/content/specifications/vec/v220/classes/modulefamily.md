...
|mandatory| [Boolean]({{< relref "boolean.md" >}}) | 0..1 | If '<i>mandatory=true</i>' applies, then exactly one <i>PartWithSubComponentsRole</i> from the <i>ModuleFamily</i> the must be contained in a valid vehicle configuration, provided that the <i>ModuleFamily</i> itself is valid for the vehicle configuration. This is implied by the fact that the <i>ModuleFamily</i> itself is a <i>ConfigurableElement</i>. If this attribute is false or not defined, then at most one <i>PartWithSubComponentsRole</i> can be selected from the <i>ModuleFamily.</i> | [ModuleFamily]({{< relref "modulefamily.md" >}}) |
